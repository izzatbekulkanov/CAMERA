...
export class VideoRTC extends HTMLElement {
    constructor() {
         ...
         * @type {string}
         */
        this.mode = this.getAttribute('mode') || 'mse,webrtc';
    ...
    set src(value) {
        ...
        if (value.startsWith('https://')) {
            ...
            value = (location.protocol === 'https:' ? 'wss://' : 'ws://') + location.host + value;
        }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        if (name === 'src' && newValue) {
            this.src = newValue;
        } else if (name === 'mode' && newValue) {
            this.mode = newValue;
        } else if (name === 'media' && newValue) {
            this.media = newValue;
        }
    }
    ...
    connectedCallback() {
        ...
        if (!this.wsURL && this.hasAttribute('src')) {
    ...
    oninit() {
        ...
        this.video.addEventListener('loadeddata', () => {
            this.dispatchEvent(new CustomEvent('loadeddata', { bubbles: true }));
        });
        this.video.addEventListener('playing', () => {
            this.dispatchEvent(new CustomEvent('playing', { bubbles: true }));
        });
        this.video.addEventListener('timeupdate', () => {
            if (this.video && this.video.currentTime > 0) {
                this.dispatchEvent(new CustomEvent('firstframe', { bubbles: true }));
            }
        });
    ...
    onconnect() {
        ...
        this.ws.addEventListener('open', () => {
            console.log(`%c[VideoRTC: ${this.streamName || this.wsURL}] 🟢 WebSocket aloqasi muvaffaqiyatli o'rnatildi`, 'color: #10b981; font-weight: bold;');
            this.onopen();
        });
        this.ws.addEventListener('error', (err) => {
            console.error(`%c[VideoRTC: ${this.streamName || this.wsURL}] ❌ WebSocket ulanish xatoligi:`, 'color: #ef4444; font-weight: bold;', err);
        });
        this.ws.addEventListener('close', (ev) => {
            if (ev.code === 1000 || this.pcState === WebSocket.OPEN) {
                console.log(`%c[VideoRTC: ${this.streamName || this.wsURL}] 🟢 WebRTC/MSE aloqasi barqaror ishlamoqda`, 'color: #10b981;');
                return;
            }
            console.warn(`%c[VideoRTC: ${this.streamName || this.wsURL}] ⚠️ WebSocket yopildi (kod: ${ev.code}, sabab: ${ev.reason || 'Kamera offline'})`, 'color: #f59e0b;');
            this.onclose();
        });
        ...
        this.onmessage['mse'] = msg => {
            ...
            sb.addEventListener('updateend', () => {
                ...
                if (!sb.updating && sb.buffered && sb.buffered.length) {
                    ...
                    // Fix: Do not use currentTime seek for small gaps as it stalls waiting for I-frames
                    if (gap > 3.0) {
